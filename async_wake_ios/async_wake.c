...
void make_dangling(mach_port_t port) {
  ...
  for (int i = 0; i < 2; i++) {
    IOConnectCallAsyncMethod(user_client,
  ...
  IOConnectCallMethod(user_client,
...
mach_port_t get_kernel_memory_rw() {
  ...
  uint32_t i;
  printf("Attempting to control the port\n");
...
void run_exploit_and_jailbreak(const char *path) {
  printf("This application is located at: %s\n", path);
  mach_port_t tfp0 = get_kernel_memory_rw();
  printf("tfp0: %x\n", tfp0);
    
  if (probably_have_correct_symbols()) {
      printf("[+]\tI have the right symbols!\n");
      jailbreak((char *)path, tfp0, 0);
      printf("[+]\tDone!\n\n\n\n");
  } else {
      printf("I don't have symbols for this, gonna try jailbreak but it could fail (if it does use LiberiOS!)\n");
      if (strstr(iphone_version, "iPhone6,1"))
